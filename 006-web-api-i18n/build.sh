...
/bin/bash -c "set -o pipefail \
  && cd ${SRC_DIR} \
  && ${TOOLCHAIN_DOTNET} build \
  && ${TOOLCHAIN_DOTNET} pack --include-symbols --include-source -o ${BUILD_DIR}/nuget"

EXIT_CODE=$?
if [[ ${EXIT_CODE} -eq 0 ]]; then
  echo "Nuget build was successful"
else
  echo "Nuget build failed with ERROR: ${EXIT_CODE}"
  exit ${EXIT_CODE}
fi
